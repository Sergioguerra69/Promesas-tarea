...
describe("Función CrearGasto y funcionamiento de objeto gasto", function () {
  it("CrearGasto: comprobación con fecha y etiquetas", function () {
    ...
    assert.isEmpty(
      gasto1.etiquetas,
      "Si el gasto se crea sin etiquetas, la propiedad 'etiquetas' debe inicializarse a un array vacío"
    );
...
`
  ...
  it("Método 'actualizarFecha' del objeto gasto", function () {
    ...
    assert.equal(
      gasto1.fecha,
      orig,
      "Si la fecha no es válida, se debe dejar sin modificar."
    );
  ...
  it("Método 'anyadirEtiquetas' del objeto gasto", function () {
    ...
    assert.lengthOf(
      gasto2.etiquetas,
      0,
      "Un gasto sin etiquetas debe inicializar su propiedad 'etiquetas' a un array vacío"
    );
    ...
    assert.lengthOf(
      gasto2.etiquetas,
      8,
      "Si las etiquetas ya existen no se deben añadir."
    );
    gasto2.anyadirEtiquetas("nueva2", "nueva9", "nueva3");
    assert.lengthOf(
      gasto2.etiquetas,
      9,
      "Si las etiquetas ya existen no se deben añadir: solo deben añadirse las que no existan previamente en el objeto gasto."
    );
  ...
  it("Método 'borrarEtiquetas' del objeto gasto", function () {
    ...
    assert.lengthOf(
      gasto4.etiquetas,
      0,
      "Si se borran todas las etiquetas la propiedad etiquetas debe ser un array vacío"
    );
  });
});
...
describe("Función anyadirGasto", function () {
  it("Añade el gasto que se pasa como parámetro a la variable global 'gastos'", function () {
    ...
    assert.equal(
      listarGastos()[0].id,
      0,
      "Al añadir un gasto se le tiene que asignar un id que se irá incrementando"
    );
    anyadirGasto(gasto2);
...
describe("Función borrarGasto", function () {
  it("Elimina de la variable global 'gastos' el gasto cuyo id se pasa como parámetro", function () {
    ...
    assert.equal(
      listarGastos()[0].id,
      0,
      "Al borrar un gasto deben quedar los que había antes"
    );
    assert.equal(
      listarGastos()[1].id,
      2,
    ...
    assert.lengthOf(
      listarGastos(),
      0,
      "Al borrar todos los gastos debe quedar un array vacío"
    );
  });
});
...
describe("Función calcularBalance", function () {
  it("Calcula el balance (presupuesto - gastos totales) a partir de los gastos almacenados en la variable global 'gastos'", function () {
    ...
    assert.equal(
      calcularBalance(),
      presupuesto - valor1 - nuevovalor2 - valor3
    );
